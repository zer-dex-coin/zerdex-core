    ...
    if (ui->radioListMode->isChecked()) {
        ...
        for (int i = 0; i < ui->treeWidget->topLevelItemCount(); i++) {
            ...
            if (model->isLockedCoin(uint256(item->text(COLUMN_TXHASH).toStdString()), item->text(COLUMN_VOUT_INDEX).toUInt())) {
                ...
                item->setIcon(COLUMN_CHECKBOX, QIcon(":/icons/lock_closed2"));
            }
            updateLabelLocked();
        }
        ui->treeWidget->setEnabled(true);
...
void CoinControlDialog::lockCoin()
{
    ...
    contextMenuItem->setIcon(COLUMN_CHECKBOX, QIcon(":/icons/lock_closed2"));
    updateLabelLocked();
}
...
void CoinControlDialog::updateLabels(WalletModel* model, QDialog* dialog)
{
    ...
    int nDisplayUnit = BitcoinUnits::ZDX;
        ...
        for(const COutput& out: coins.second) {
            ...
            if (fMultiSigUTXO) {
                ...
                if (!fMultisigEnabled) {
                    ...
                    itemOutput->setIcon(COLUMN_CHECKBOX, QIcon(":/icons/lock_closed2"));
                }
            } else {
            ...
            if (ExtractDestination(out.tx->vout[out.i].scriptPubKey, outputAddress)) {
                ...
                // if listMode or change => show Zdx address. In tree mode, address is not shown again for direct wallet address outputs
