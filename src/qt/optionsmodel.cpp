...
void OptionsModel::Init()
{
    ...
    if (!settings.contains("nDisplayUnit"))
        settings.setValue("nDisplayUnit", BitcoinUnits::ZDX);
    ...
    if (!settings.contains("nAnonymizeZdxAmount"))
        settings.setValue("nAnonymizeZdxAmount", 1000);

    nAnonymizeZdxAmount = settings.value("nAnonymizeZdxAmount").toLongLong();
...
// Wallet
    ...
    if (settings.contains("nAnonymizeZdxAmount"))
        SoftSetArg("-anonymizezdxamount", settings.value("nAnonymizeZdxAmount").toString().toStdString());
...
void OptionsModel::Reset()
{
    ...
    settings.clear();
    resetSettings = true; // Needed in zdx.cpp during shotdown to also remove the window positions
...
QVariant OptionsModel::data(const QModelIndex& index, int role) const
{
    if (role == Qt::EditRole) {
        ...
        case AnonymizeZdxAmount:
            return QVariant(nAnonymizeZdxAmount);
        case Listen:
