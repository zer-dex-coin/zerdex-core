...
// Copyright (c) 2017-2018 The Zdx developers
// Distributed under the MIT/X11 software license, see the accompanying
// file COPYING or http://www.opensource.org/licenses/mit-license.php.

#include "zzdxcontroldialog.h"
#include "ui_zzdxcontroldialog.h"
...
std::list<std::string> ZZdxControlDialog::listSelectedMints;
std::list<CZerocoinMint> ZZdxControlDialog::listMints;

ZZdxControlDialog::ZZdxControlDialog(QWidget *parent) :
    QDialog(parent),
    ui(new Ui::ZZdxControlDialog),
    model(0)
{
    ui->setupUi(this);
    listMints.clear();
...
ZZdxControlDialog::~ZZdxControlDialog()
{
    delete ui;
}

void ZZdxControlDialog::setModel(WalletModel *model)
{
    this->model = model;
    updateList();
}
...
void ZZdxControlDialog::updateList()
{
...
void ZZdxControlDialog::updateSelection(QTreeWidgetItem* item, int column)
{
...
void ZZdxControlDialog::updateLabels()
{
    int64_t nAmount = 0;
    for (const CZerocoinMint mint : listMints) {
    ...
    ui->labelZZdx_int->setText(QString::number(nAmount));
    ui->labelQuantity_int->setText(QString::number(listSelectedMints.size()));

    //update PrivacyDialog labels
    privacyDialog->setZZdxControlLabels(nAmount, listSelectedMints.size());
}

std::vector<CZerocoinMint> ZZdxControlDialog::GetSelectedMints()
{
...
void ZZdxControlDialog::ButtonAllClicked()
{
    ui->treeWidget->blockSignals(true);
