...
#if defined(NDEBUG)
#error "ZDX cannot be compiled without assertions."
#endif
...

    {
        ...
        if(tx.IsZerocoinSpend()){
            ...
            if (IsTransactionInChain(tx.GetHash(), nHeightTx))
                return state.Invalid(error("AcceptToMemoryPool : zZdx spend tx %s already in block %d", tx.GetHash().GetHex(), nHeightTx),
            ...
            for (const CTxIn& txIn : tx.vin) {
                ...
                if (IsSerialInBlockchain(spend.getCoinSerialNumber(), nHeightTx))
                    return state.Invalid(error("%s : zZdx spend with serial %s is already in block %d\n",
                ...
                if (!spend.HasValidSerial(Params().Zerocoin_Params()))
                    return state.Invalid(error("%s : zZdx spend with serial %s from tx %s is not in valid range\n",
...
int64_t GetBlockValue(int nHeight)
{
    ...
    if (nHeight == 1) {
        nSubsidy = 820000 * COIN;  
    } else if(nHeight >=2 && nHeight <= Params().LAST_POW_BLOCK()) { //PoW phase
        nSubsidy = 0 * COIN;
    } else if(nHeight > Params().LAST_POW_BLOCK() && nHeight < 20001) { //PoS phase
        nSubsidy = 5 * COIN;
    } else if(nHeight >= 20001 && nHeight < 40001) {
        nSubsidy = 7 * COIN;
    } else if(nHeight >= 40001 && nHeight < 60001) {
        nSubsidy = 9 * COIN;
    } else if(nHeight >= 60001 && nHeight < 80001) {
        nSubsidy = 11 * COIN;
    } else if(nHeight >= 80001 && nHeight < 100001) {
        nSubsidy = 13 * COIN;
    } else if(nHeight >= 100001 && nHeight < 140001) {
        nSubsidy = 15 * COIN;
    } else if(nHeight >= 140001 && nHeight < 180001) {
        nSubsidy = 17 * COIN;
    } else if(nHeight >= 180001 && nHeight < 220001) {
        nSubsidy = 19 * COIN;
    } else if(nHeight >= 220001 && nHeight < 260001) {
        nSubsidy = 21 * COIN;
    } else if(nHeight >= 260001 && nHeight < 300001) {
        nSubsidy = 22 * COIN;
    } else if(nHeight >= 300001 && nHeight < 340001) {
        nSubsidy = 24 * COIN;
    } else if(nHeight >= 340001 && nHeight < 380001) {
        nSubsidy = 26 * COIN;
    } else if(nHeight >= 380001 && nHeight < 420001) {
        nSubsidy = 28 * COIN;
    } else if(nHeight >= 420001 && nHeight < 460001) {
        nSubsidy = 30 * COIN;
    } else if(nHeight >= 460001 && nHeight < 500001) {
        nSubsidy = 32 * COIN;
        ...
        nSubsidy = 55 * COIN;
    } else if(nHeight >= 1000001 && nHeight < 1200001) {
        nSubsidy = 40 * COIN;
    } else if(nHeight >= 1200001 && nHeight < 1400001) {
        nSubsidy = 30 * COIN;
    } else if(nHeight >= 1400001 && nHeight < 1600001) {
        nSubsidy = 20 * COIN;
    } else if(nHeight >= 1600001 && nHeight < 2000001) {
        nSubsidy = 9 * COIN;
    } else if(nHeight >= 2000001 && nHeight < 2400001) {
        nSubsidy = 8 * COIN;
    } else if(nHeight >= 2400001 && nHeight < 2800001) {
        nSubsidy = 7 * COIN;
    } else if(nHeight >= 2800001 && nHeight < 3200001) {
        nSubsidy = 6 * COIN;
    } else if(nHeight >= 3200001 && nHeight < 3600001) {
        nSubsidy = 5.5 * COIN;
    } else {
        nSubsidy = 5 * COIN;  //everything else
    }
    return nSubsidy;
}
...
int64_t GetMasternodePayment(int nHeight, int64_t blockValue, int nMasternodeCount)
{
    ...
    if (nHeight <= Params().LAST_POW_BLOCK()) {
          ...
          ret = blockValue  / 100 * 80; //75% for MN
...
bool DisconnectBlock(CBlock& block, CValidationState& state, CBlockIndex* pindex, CCoinsViewCache& view, bool* pfClean)
{
    ...
    for (int i = block.vtx.size() - 1; i >= 0; i--) {
         ...
         * note we only undo zerocoin databasing in the following statement, value to and from Zdx
...
{
    RenameThread("zdx-scriptch");
    scriptcheckqueue.Thread();
}

void RecalculateZZDXMinted()
{
...
void RecalculateZZDXSpent()
{
    ...
    while (true) {
        ...
        //Remove spends from zZDX supply
...
bool RecalculateZDXSupply(int nHeightStart)
{
...
bool ConnectBlock(const CBlock& block, CValidationState& state, CBlockIndex* pindex, CCoinsViewCache& view, bool fJustCheck, bool fAlreadyChecked)
{
                ...
                if (zerocoinDB->ReadCoinSpend(spend.getCoinSerialNumber(), hashTxFromDB)) {
                    if(IsSerialInBlockchain(spend.getCoinSerialNumber(), nHeightTxSpend)) {
                        if(!fVerifyingBlocks || (fVerifyingBlocks && pindex->nHeight > nHeightTxSpend))
                            return state.DoS(100, error("%s : zZdx with serial %s is already in the block %d\n",
    ...
    //LogPrintf("XX69----------> ConnectBlock(): nValueOut: %s, nValueIn: %s, nFees: %s, nMint: %s zZdxSpent: %s\n",
...
void static UpdateTip(CBlockIndex* pindexNew)
{
    ...
    // If turned on AutoZeromint will automatically convert ZDX to zZDX
...
bool CheckBlock(const CBlock& block, CValidationState& state, bool fCheckPOW, bool fCheckMerkleRoot, bool fCheckSig)
{
    ...
    for (const CTransaction& tx : block.vtx) {
        ...
        // double check that there are no double spent zZdx spends in this block
        if (tx.IsZerocoinSpend()) {
            for (const CTxIn txIn : tx.vin) {
                if (txIn.scriptSig.IsZerocoinSpend()) {
                    libzerocoin::CoinSpend spend = TxInToZerocoinSpend(txIn);
                    if (count(vBlockSerials.begin(), vBlockSerials.end(), spend.getCoinSerialNumber()))
                        return state.DoS(100, error("%s : Double spending of zZdx serial %s in block\n Block: %s",
...
bool ProcessNewBlock(CValidationState& state, CNode* pfrom, CBlock* pblock, CDiskBlockPos* dbp)
{
    ...
    if (nMints || nSpends)
        LogPrintf("%s : block contains %d zZdx mints and %d zZdx spends\n", __func__, nMints, nSpends);
...
bool LoadExternalBlockFile(FILE* fileIn, CDiskBlockPos* dbp)
{
    ...
    try {
        ...
        while (!blkdat.eof()) {
            ...
            try {
                // locate a header
                unsigned char buf[MESSAGE_STZDX_SIZE];
                blkdat.FindByte(Params().MessageStart()[0]);
                nRewind = blkdat.GetPos() + 1;
                blkdat >> FLATDATA(buf);
                if (memcmp(buf, Params().MessageStart(), MESSAGE_STZDX_SIZE))
                    continue;
                // read size
                blkdat >> nSize;
...
bool static ProcessMessage(CNode* pfrom, string strCommand, CDataStream& vRecv, int64_t nTimeReceived)
{
    ...
    if (strCommand == "version") {
        ...
        // Zdx: We use certain sporks during IBD, so check to see if they are
...
bool ProcessMessages(CNode* pfrom)
{
    ...
    while (!pfrom->fDisconnect && it != pfrom->vRecvMsg.end()) {
        ...
        if (memcmp(msg.hdr.pchMessageStart, Params().MessageStart(), MESSAGE_STZDX_SIZE) != 0) {
            LogPrintf("PROCESSMESSAGE: INVALID MESSAGESTZDX %s peer=%d\n", SanitizeString(msg.hdr.GetCommand()), pfrom->id);
            fOk = false;
            break;
        }
