...
Value getinfo(const Array& params, bool fHelp)
{
    if (fHelp || params.size() != 0)
        throw runtime_error(
            ...
            "  \"paytxfee\": x.xxxx,         (numeric) the transaction fee set in zdx/kb\n"
            "  \"relayfee\": x.xxxx,         (numeric) minimum relay fee for non-free transactions in zdx/kb\n"
...
Value spork(const Array& params, bool fHelp)
{
    if (params.size() == 1 && params[0].get_str() == "show") {
        Object ret;
        for (int nSporkID = SPORK_STZDX; nSporkID <= SPORK_END; nSporkID++) {
...
Value validateaddress(const Array& params, bool fHelp)
{
    if (fHelp || params.size() != 1)
        throw runtime_error(
            "validateaddress \"zdxaddress\"\n"
            "\nReturn information about the given zdx address.\n"
            "\nArguments:\n"
            "1. \"zdxaddress\"     (string, required) The zdx address to validate\n"
            "\nResult:\n"
            "{\n"
            "  \"isvalid\" : true|false,         (boolean) If the address is valid or not. If not, this is the only property returned.\n"
            "  \"address\" : \"zdxaddress\", (string) The zdx address validated\n"
...
#ifdef ENABLE_WALLET
        // Case 1: Zdx address and we have full public key:
...
Value createmultisig(const Array& params, bool fHelp)
{
    if (fHelp || params.size() < 2 || params.size() > 2) {
        string msg = "createmultisig nrequired [\"key\",...]\n"
                     ...
                     "2. \"keys\"       (string, required) A json array of keys which are zdx addresses or hex-encoded public keys\n"
                     "     [\n"
                     "       \"key\"    (string) zdx address or hex-encoded public key\n"
                     "       ,...\n"
                     "     ]\n"
...
Value verifymessage(const Array& params, bool fHelp)
{
    if (fHelp || params.size() != 3)
        throw runtime_error(
            "verifymessage \"zdxaddress\" \"signature\" \"message\"\n"
            "\nVerify a signed message\n"
            "\nArguments:\n"
            "1. \"zdxaddress\"  (string, required) The zdx address to use for the signature.\n"
